...
//  MedBook
//
//  Created by PRITESH SINGH on 08/08/23.
//
...
class MBSignupViewController: UIViewController {
    ...
    private var repositoryParam: MBRepositoryParam?
    ...
    private func initializeFactoryWithParam() {
        ...
        self.repositoryParam = MBRepositoryParam(apiType: .CountryList)
        if let _repoParam = self.repositoryParam {
            self.factory = MBFactory(params: _repoParam)
        }
        self.refreshData()
    }
    ...
    private func getRepository() -> MBDataRepository? {
        return self.factory?.getRepository()
    }
